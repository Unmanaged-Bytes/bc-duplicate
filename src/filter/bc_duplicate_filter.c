...
static bool bc_duplicate_filter_count_and_split(bc_allocators_context_t* memory_context, const char* list, char** out_buffer, char*** out_patterns,
                                                size_t* out_count)
{
                                ...
                                bc_duplicate_filter_t** out_filter)
{
    ...
    if (!bc_allocators_pool_allocate(memory_context, sizeof(bc_duplicate_filter_t), (void**)&filter)) {
        return false;
    }
    bc_core_zero(filter, sizeof(*filter));

    if (!bc_duplicate_filter_count_and_split(memory_context, include_list, &filter->include_buffer, &filter->include_patterns,
                                             &filter->include_count)) {
        bc_allocators_pool_free(memory_context, filter);
        return false;
    }

    if (!bc_duplicate_filter_count_and_split(memory_context, exclude_list, &filter->exclude_buffer, &filter->exclude_patterns,
...
static bool bc_duplicate_filter_basename_matches_any(char** patterns, size_t count, const char* basename)
{
...
bool bc_duplicate_filter_accepts_file(const bc_duplicate_filter_t* filter, const char* basename)
{
    ...
    if (filter->exclude_count > 0 && bc_duplicate_filter_basename_matches_any(filter->exclude_patterns, filter->exclude_count, basename)) {
        return false;
    }
    if (filter->include_count > 0 && !bc_duplicate_filter_basename_matches_any(filter->include_patterns, filter->include_count, basename)) {
        return false;
    }
    return true;
}
