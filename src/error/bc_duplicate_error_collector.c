...
typedef struct bc_duplicate_error_record {
    char* path;
    const char* stage;
    int errno_value;
} bc_duplicate_error_record_t;

struct bc_duplicate_error_collector {
...
{
    bc_duplicate_error_collector_t* collector = NULL;
    if (!bc_allocators_pool_allocate(memory_context, sizeof(bc_duplicate_error_collector_t), (void**)&collector)) {
        return false;
    }
    bc_core_zero(collector, sizeof(*collector));

    if (!bc_containers_vector_create(memory_context, sizeof(bc_duplicate_error_record_t), BC_DUPLICATE_ERROR_INITIAL_CAPACITY,
                                     BC_DUPLICATE_ERROR_MAX_CAPACITY, &collector->records)) {
...
void bc_duplicate_error_collector_destroy(bc_allocators_context_t* memory_context, bc_duplicate_error_collector_t* collector)
{
    ...
    for (size_t index = 0; index < record_count; ++index) {
        bc_duplicate_error_record_t record;
...
bool bc_duplicate_error_collector_flush_to_stderr(const bc_duplicate_error_collector_t* collector)
{
    ...
    for (size_t index = 0; index < record_count; ++index) {
        ...
        fprintf(stderr, "bc-duplicate: %s: %s: %s\n", record.stage, record.path, reason);
    }
    return true;
}
