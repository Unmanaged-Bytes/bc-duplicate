...
static void test_uring_batch_rejects_file_that_grew_between_stat_and_read(void** state)
{
    ...
    char root[256];
    snprintf(root, sizeof(root), "/tmp/bc_dup_ring_growth_%d_%ld", (int)getpid(), (long)time(NULL));
    ...
    size_t ring_size = bc_duplicate_reader_ring_struct_size();
    bc_duplicate_reader_ring_t* ring = (bc_duplicate_reader_ring_t*)malloc(ring_size);
    assert_non_null(ring);
    assert_true(bc_duplicate_reader_ring_init(ring));

    consumer_state_t consumer = {0};
    bc_duplicate_reader_batch_item_t item = {0};
    item.absolute_path = file_path;
    item.file_size = stale_size;
    item.consumer_context = &consumer;

    assert_true(bc_duplicate_reader_consume_batch(ring, &item, 1U, record_consumer));
    assert_false(item.success);

    bc_duplicate_reader_ring_destroy(ring);
    free(ring);
...
static void test_uring_batch_accepts_file_whose_size_matches(void** state)
{
    ...
    char root[256];
    snprintf(root, sizeof(root), "/tmp/bc_dup_ring_growth_ok_%d_%ld", (int)getpid(), (long)time(NULL));
    assert_int_equal(mkdir(root, 0755), 0);
